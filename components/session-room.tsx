...
type Signal = {
  type: "offer" | "answer" | "ice" | "hello" | "ready";
  from: string;
  payload: any;
};
...
export default function SessionRoom({
  ...
  const [seconds, setSeconds] = useState(() =>
    Math.max(
      0,
      Math.floor((new Date(room.endsAt).getTime() - Date.now()) / 1000),
    ),
  );
  ...
  // Ask for camera/mic permission as soon as the room opens, so the browser
  // shows its native allow/block prompt right away instead of waiting for
  // the user to press the camera button.
  useEffect(() => {
    void startCamera();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`session-media-${room.sessionId}`, {
        config: { broadcast: { self: false } },
      })
      .on(
        "broadcast",
        { event: "signal" },
        async ({ payload }: { payload: Signal }) => {
          ...
          try {
            if (payload.type === "hello") {
              ...
              await channel.send({
                type: "broadcast",
                event: "signal",
                payload: { type: "ready", from: room.me.id, payload: null },
              });
              if (
      ...
      .subscribe(async (subStatus) => {
        ...
        await channel.send({
          type: "broadcast",
          event: "signal",
          payload: { type: "hello", from: room.me.id, payload: null },
        });
        if (
          remoteReady &&
          pc.getSenders().some((s) => s.track) &&
  ...
  async function syncTracks() {
    ...
    for (const kind of ["video", "audio"] as const) {
      ...
      if (sender) {
        if (sender.track?.id !== desired?.id)
          await sender.replaceTrack(desired);
      } else if (desired) {
  ...
  useEffect(() => {
    void syncTracks().catch((e) => console.error("syncTracks error", e));
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        {
          ...
          filter: `session_id=eq.${room.sessionId}`,
        },
        (payload) => {
  ...
  useEffect(() => {
    async function beat() {
      ...
      if (row) {
        ...
        setSeconds(Number(row.seconds_left ?? 0));
      }
    }
    void beat();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data } = await supabase.rpc("heartbeat_live_session", {
          p_session_id: room.sessionId,
        });
        const row = Array.isArray(data) ? data[0] : data;
        if (row) {
  ...
  function toggleMic() {
    setMic((v) => {
      cameraStream?.getAudioTracks().forEach((t) => (t.enabled = !v));
      return !v;
    });
  }
      ...
      <div className="session-room-grid">
        <section className="session-stage">
          ...
          <div className="session-controls">
            <button
              className={`media-control ${camera ? "active" : ""}`}
              onClick={camera ? stopCamera : startCamera}
            >
              <Camera size={17} />
              {camera ? "إيقاف الكاميرا" : "الكاميرا"}
            </button>
            <button
              className={`media-control ${mic ? "active" : ""}`}
              onClick={toggleMic}
            ...
            <button
              className={`media-control ${screenStream ? "active" : ""}`}
              onClick={screenStream ? stopScreen : startScreen}
            >
              <MonitorUp size={17} />
              {screenStream ? "إيقاف المشاركة" : "مشاركة الشاشة"}
            </button>
          </div>
        ...
        <aside className="session-chat">
          ...
          <div className="session-messages">
            ...
            {messages.map((m) => (
              <div
                key={m.id}
                className={`session-message ${m.senderId === room.me.id ? "mine" : ""}`}
              >
                <span>
                  {m.senderId === room.me.id ? "أنت" : room.other.name}
                </span>
                <p>{m.content}</p>
              </div>
            ))}
